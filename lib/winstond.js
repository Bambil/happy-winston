...
class Winstond extends winston.Logger {
  constructor (options) {
    ...
    this.methods = options.method || ['collect', 'query']
    ...
    this.server.route({
      ...
      handler: (request, reply) => {
        if (request.payload.method === 'collect' && 'collect' in this.methods) {
        ...
        } else if (request.payload.method === 'query' && 'query' in this.methods) {
